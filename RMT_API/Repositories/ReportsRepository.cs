...
namespace RMT_API.Repositories
{
	public class ReportsRepository(ApplicationDBContext _context) : IReportsRepository
	{
		...
		public async Task<IEnumerable<ResourceReports>> GetResourceReportsAsync(string filterType = "", string searchName = "", int pageNumber = 1, int pageSize = 10)
		{
			// Define the stored procedure call and pass parameters
			var filterTypeParam = filterType;  // Ensure we pass an empty string if null
			var searchNameParam = searchName;
		...
		public async Task<IEnumerable<ProjectReports>> GetProjectReportsAsync(string filterType = "", string searchName = "", int pageNumber = 1, int pageSize = 10)
		{
			// Define the stored procedure call and pass parameters
			var filterTypeParam = filterType;  // Ensure we pass an empty string if null
			var searchNameParam = searchName;
			...
			// Using FromSqlRaw for executing the stored procedure
			var resourceReports = await _context.ProjectReports
				.FromSqlRaw("Exec SearchProjectReports @FilterType = {0}, @SearchName  = {1}, @PageNumber  = {2}, @PageSize = {3} ",
					filterTypeParam, searchNameParam, pageNumberParam, pageSizeParam)
				.ToListAsync();
		...
		public async Task<IEnumerable<SupplierReports>> GetSupplierReportsAsync(string filterType = "", string searchName = "", int pageNumber = 1, int pageSize = 10)
		{
			// Define the stored procedure call and pass parameters
