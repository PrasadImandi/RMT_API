...
const EditClient = () => {
  ...
  // Fetch current client data and reset form values
  useEffect(() => {
    const fetchClient = async () => {
      try {
        ...
        form.reset({
          ...
          endDate: clientData?.endDate ? new Date(clientData.endDate) : new Date(),
          regionID: clientData?.regionID || 0,
          stateID: clientData?.stateID || 0,
          locationID: clientData?.locationID || "",
          pincodeID: clientData?.pincodeID || 0,
          address: clientData?.address || "",
          spocName: clientData?.spocName || "",
          spocContactNumber: clientData?.spocContactNumber || "",
          spocEmail: clientData?.spocEmail || "",
  ...
  // Submit handler for updating the client
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    console.log("Form Submitted", updatedClient);
    try {
      const res = await api.put(`/Client/${params.id}`, updatedClient);
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input as="textarea" placeholder="Enter address" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input as="textarea" placeholder="Enter notes" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
