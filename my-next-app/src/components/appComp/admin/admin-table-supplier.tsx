...
const AdminTableSupplier = () => {
  ...
  const { data: resources = [], isLoading } = useQuery<SupplierRow[]>({
    queryKey: ["resources"],
  ...
  const deactivateProject = useMutation({
    mutationFn: SupplierApi.deactivateSuppliers,
    onSuccess: () => {
  ...
  const filteredData = resources.filter((row) => {
    ...
    const matchesSearch =
      ...
      (row.tan ?? "").toLowerCase().includes(lowerSearchTerm) ||
      (row.pan ?? "").toLowerCase().includes(lowerSearchTerm) ||
      (row.gst ?? "").toLowerCase().includes(lowerSearchTerm);
